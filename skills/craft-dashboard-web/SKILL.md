---
name: craft-dashboard-web
description: Design and implement clear, data-driven responsive web dashboards for SaaS, admin, analytics, operations, finance, and internal tools. Use when building or redesigning dashboard home screens, sidebars, tables, charts, filters, search, bulk actions, drill-downs, alerts, settings, or dense decision surfaces. Let the user's data and decision drive the UI; cover progressive disclosure, invisible UI, onboarding, permissions, states, accessibility, responsive behavior, and proof. Do not treat dashboard polish as evidence of functional quality.
---

# Craft Dashboard Web

Craft a dashboard as an instrument for scanning, comparing, deciding, and
acting. Let the data, user decisions, and workflow drive the shape of the UI;
do not decorate a grid of cards and call it a dashboard.
...
## 9. Implement without breaking reality

Use real data fixtures and preserve query, pagination, permission, mutation,
audit, analytics, export, and provider behavior. Never create fake chart
values, fake success, or impossible state combinations to make the dashboard
look finished.
...
Plan → Build → Run → Screenshot/Record → Critique → Refine → Verify

Observe the primary decision journey plus typical, dense, empty, no-results,
loading, stale, permission, error, rollback, and interrupted states. Test
search/filter/sort, selection, bulk action, chart interpretation, drill-down,
browser back/refresh/deep link, keyboard, screen reader, reduced motion,
responsive widths, semantic light/dark color, contrast, tooltip timing,
no-hover equivalents, and performance. Test the interaction matrix, not only
the default cards.

Report exact routes, data fixtures, roles, query states, viewport/browser
matrix, screenshots/recordings, checks run, data/provider gates, and unknowns.
Do not call a dashboard complete because the default cards look polished.

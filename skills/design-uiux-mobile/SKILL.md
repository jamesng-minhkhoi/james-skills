...
wireframes, high-fidelity screen direction, content hierarchy, component/state
specifications, motion/gesture contracts, responsive rules, prototypes, and a
clear handoff. Do not edit application code, schemas, APIs, stores,
permissions, analytics, or native configuration. Use
...
use the [regression evaluation](evals/design-uiux-mobile-regression.md) to compare
...
## 7. Specify the full interaction surface

Specify the states a user can actually experience:
...
- For optimistic changes, define the success assumption, pending treatment,
  rollback, retry, and stale-data behavior before handoff. Describe what the
  product must do; do not implement the behavior here.
...
- First choose whether motion is needed. If it is, classify it as within-page
  ...
  and gesture system reference to write a motion contract before handoff.
...
Use existing platform conventions, design tokens, and component contracts as
constraints. Split the design by meaningful visual regions, not arbitrary
component boundaries. Document required states, content assumptions, route
destinations, accessibility behavior, motion, and unresolved product or data
questions. Keep the handoff implementation-agnostic and reviewable.

Do not change application code or claim that navigation, data, permissions,
analytics, or native behavior works. Hand implementation to the repository's
...
- Render or prototype the primary journey plus representative non-happy states
  in the design/prototype surface or closest available mobile harness.
...
- Report the design artifact, flow/states, content fixture, viewport/device,
  interaction checkpoints, decisions, open questions, and implementation gates
  intentionally left to engineering. Do not call product behavior complete
  from a static design or prototype.
